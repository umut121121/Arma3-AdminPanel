...
function guid($max, $pid)
{
    ...
    $return = 'GUID 32 bit php ile kullanılamaz!';
...
function logs($perms, $column, $pid, $user, $dbcon, $player, $val)
{
    if ($perms == '1') {
        if ($val != $player->$column) {
            $message = 'Admin '.$user.' değişti '.utf8_encode($player->name).'('.$pid.')'.' '.$column.' -den '.$player->$column.' -e doğru '.$val;
        ...
        if ($val != $player->$column) {
            $message = 'Admin '.$user.' değişmeye çalıştı '.utf8_encode($player->name).'('.$pid.')'.' '.$column.' -den '.$player->$column.' -e doğru '.$val;
            logIt($user, $message, $dbcon);
