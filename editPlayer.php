...
?>
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h1 style = "margin-top: 70px">Oynatıcıyı Düzenle</h1>
		  <p class="page-header">Panelin oyuncu menüsünü düzenle, değerleri daha derinlemesine değiştirmenizi sağlar.</p>
          <div id="alert-area"></div>
		  <form action='editPlayer.php' method='post'>
		  <div class="btn-group" role="group" aria-label="...">
		  <input class = 'btn btn-primary btn-outline' type='submit' name='remove' value='Civ Lisanslarını Sıfırla'>
	   	  </div>
          <div class="btn-group" role="group" aria-label="...">
          <input class = 'btn btn-primary btn-outline' type='submit' name='give' value='Tüm Civ Lisanslarını Verin'>
          </div>
          <input type=hidden name=hidden value= <?php echo $uidPlayer; ?> >
          <input type=hidden name=guid value= <?php echo $guidPlayer; ?> >
          <div class="btn-group" role="group" aria-label="...">
          <button type="button" class="btn btn-primary btn-outline" data-toggle="modal" data-target="#myModal">Oyuncu Envanteri</button>
          </form>
    </div>
          <br>
...
<div class="modal-dialog modal-lg">
  <div class="modal-content">
    <div class="modal-header">
      ...
      <h4 class="modal-title" id="myModalLabel">Oyuncu Envanteri</h4>
    </div>
    <div class="modal-body">

        <div class='panel panel-info'>
            <div class='panel-heading'>
                <h3 class='panel-title'>Sivil Envanter</h3>
            </div>
        ...
        <div class='panel panel-info'>
            <div class='panel-heading'>
                <h3 class='panel-title'>Polis Envanter</h3>
            </div>
            <div class='panel-body'>
        ...
        <div class='panel panel-info'>
            <div class='panel-heading'>
                <h3 class='panel-title'>Doktor Envanter</h3>
            </div>
            <div class='panel-body'>
...
echo '<center><h4>Oyuncu ID: '.$pid.'</h3></center>';
echo '<center><h4>GUID: '.$guidPlayer.'</h3></center>';
echo '<center><h4>Banka: $'.$player->bankacc.'</h3></center>';
echo '<center><h4>Nakit: $'.$player->cash.'</h3></center>';
echo '<center><h4>Polis Level: '.$player->coplevel.'</h3></center>';
echo '<center><h4>Doktor Level: '.$player->mediclevel.'</h3></center>';
...
echo "<div class='panel panel-info'>
  <div class='panel-heading'>
    <h3 class='panel-title'>Oyunucuyu Düzenle</h3>
  </div>
  <div class='panel-body'>";
...
echo "<div id ='civlic'>";
  ...
  if ($player->civ_licenses !== '"[]"' && $player->civ_licenses !== '') {
      ...
      echo "<div class='panel panel-info'>
  <div class='panel-heading'>
    <h3 class='panel-title'>Civ Lisansları</h3>
  </div>
  <div class='panel-body'>";
...
echo "<div id ='civlic1'>";
  if ($player->med_licenses !== '"[]"' && $player->med_licenses !== '[]') {
      ...
      echo "<div class='panel panel-info'>
  <div class='panel-heading'>
    <h3 class='panel-title'>Doktor Lisanslar</h3>
  </div>
  <div class='panel-body'>";
...
echo "<div id ='civlic2'>";
  if ($player->cop_licenses !== '"[]"' && $player->cop_licenses !== '') {
      ...
      echo "<div class='panel panel-info'>
  <div class='panel-heading'>
    <h3 class='panel-title'>Polis Lisanslar</h3>
  </div>
  <div class='panel-body'>";
...
if (isset($_POST['remove'])) {
    if ($staffPerms['editPlayer'] == '1') {
        ...
        $message = 'Admin '.$user.' tüm lisansları kaldırmıştır '.utf8_encode($player->name).'('.$pid.')';
...
if (isset($_POST['give'])) {
    ...
    if ($staffPerms['editPlayer'] == '1') {
        ...
        $message = 'Admin '.$user.' tüm lisansları şu adrese ekledi: '.utf8_encode($player->name).'('.$pid.')';
        logIt($user, $message, $dbcon);
    }
}
?>
